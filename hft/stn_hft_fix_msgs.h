...
#define __STN_HFT_FIX_MSGS_H_
	...
	static uint8_t *FIX_4_2_CheckSumTemplate = (uint8_t*)"%s10=%s%c";


	static uint8_t *FIX_4_2_Generic_Msg_Template=(uint8_t*)"8=FIX.4.2%c"
												 "9=%04u%c"
												 "35=%c%c"			 /*Message type*/
	...
	static uint8_t *FIX_4_2_Login_Template = 
										(uint8_t*)"57=ADMIN%c"  // -- TargetSubID.  "ADMIN" reserved for administrative messages not intended for a specific user | 
	...
	static uint8_t *FIX_4_2_Order_Entry_Template = 
										(uint8_t*)"21=%c%c"		// Instructions for order handling on Broker trading floor 
	...
	static uint8_t *FIX_4_2_Order_Replace_Template = 
										(uint8_t*)"11=%s%c"		// Client Side Order Id
